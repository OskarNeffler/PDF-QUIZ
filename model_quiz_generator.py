...
Questions:"""
    ...
    def ask_direct_question(self, question: str, max_length: int = 150) -> str:
        """Direct question answering - chatbot functionality
        
        Args:
            question: User's question to ask the T5 model
            max_length: Maximum length of the answer
            
        Returns:
            T5 model's answer to the question
        """
        try:
            print(f"💬 T5 Chatbot answering: {question}")
            ...
            prompts = [
                ...
                f"explain: {question}",
                f"{question}",
                f"respond to: {question}"
            ]
            
            best_answer = None
            best_score = 0
            
            # Try different prompts and pick the best answer
            for prompt in prompts:
                inputs = self.tokenizer(
                    prompt,
                    max_length=512,
                ...
                score = self._score_answer_quality(answer, question)
                
                if score > best_score and len(answer.strip()) > 3:
                    best_score = score
                    best_answer = answer.strip()
            
            # Post-process the answer
            if best_answer:
                best_answer = self._clean_answer(best_answer)
                print(f"🎯 T5 Answer (score: {best_score:.2f}): {best_answer}")
                return best_answer
            else:
                return "Jag förstår inte riktigt din fråga. Kan du försöka omformulera den?"
                
        except Exception as e:
            print(f"❌ Error in direct question answering: {e}")
            return "Tyvärr uppstod ett fel när jag försökte svara på din fråga."
    
    def _score_answer_quality(self, answer: str, question: str) -> float:
        """Score the quality of an answer (simple heuristic)"""
        ...
        # 1. Answer should be substantial but not too long
        if 10 <= len(answer.split()) <= 50:
            score += 0.3
        elif 5 <= len(answer.split()) < 10:
            score += 0.2
        
        # 2. Answer should relate to the question (word overlap)
        question_words = set(question_lower.split())
        ...
        # 3. Answer should not just repeat the question
        if answer_lower != question_lower:
            score += 0.2
        
        # 4. Prefer answers that sound natural (basic checks)
        if not answer.startswith(question[:10]):  # Not just repeating question start
            score += 0.1
        
        # 5. Bonus for proper sentence structure
